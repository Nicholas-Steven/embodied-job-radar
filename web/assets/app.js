(() => {
  ...
  function jobCard(job) {
    ...
    const facts = `${cities} · ${esc(job.degree || '未公开')} · ${job.graduate_year?.length ? job.graduate_year.join('/') + '届' : '届次未公开'} · ${esc(job.job_type)}`;
    ...
    return `<article class="job-card" data-id="${esc(job.id)}"><div class="job-main"><div class="card-company-row"><div class="company-wrap"><a class="company-link" href="${companyHref}">${esc(job.company)} →</a><div class="badge-row">${trustBadge(job)}${job.first_seen === new Date().toISOString().slice(0,10) ? '<span class="badge">NEW</span>' : ''}${job.status === 'closing_soon' ? '<span class="badge warn">CLOSING SOON</span>' : ''}</div></div><div class="tag-row">${tags}</div></div><div class="card-title-row"><h3>${esc(job.title)}</h3><div class="job-meta"><span>${facts}</span>${salary ? `<span class="salary-pill">薪资 ${esc(salary)}</span>` : ''}</div></div><div class="dates"><span>发布 ${fmtDate(job.published_date)}</span><span>截止 ${fmtDate(job.deadline)}</span><span>最后核验 ${fmtDate(job.last_verified)}</span><span>来源 ${esc(job.source_name)}</span></div><div class="analysis-disclosure"><details><summary>查看匹配分析 <span class="disclosure-arrow">⌄</span></summary><div class="analysis"><div><h4>为什么适合我</h4><ul class="reasons">${reasons}</ul></div><div><h4>我的短板</h4><ul class="gaps">${gaps}</ul></div></div></details></div></div><aside class="score-column ${state.showScores ? '' : 'is-hidden'}">${scoreColumn}<a class="apply-button ${job.official_apply_url ? '' : 'secondary'}" href="${esc(link)}" target="_blank" rel="noopener noreferrer">${job.official_apply_url ? '官网投递' : '查看来源'} →</a></aside></article>`;
  }
